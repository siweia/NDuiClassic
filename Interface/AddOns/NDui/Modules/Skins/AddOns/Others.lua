...
local function reskinMHDropdown(frame)
	frame.Button = frame.MenuButton
	frame:SetHeight(32)
	B.ReskinDropDown(frame)
end

local function reskinMHTab(frame)
	local bg = B.ReskinTab(frame)
	bg:SetPoint("TOPLEFT", 5, 0)
end

function S:MeetingHorn()
	B.ReskinPortraitFrame(MeetingHornMainPanel)
	MeetingHornMainPanel.PortraitFrame:SetAlpha(0)
	for _, tab in pairs(MeetingHornMainPanel.Tabs) do
		B.ReskinTab(tab)
	end

	local feedBack = MeetingHornMainPanel.FeedBack
	B.Reskin(feedBack.AcceptButton)
	B.Reskin(feedBack.CancelButton)
	B.ReskinScroll(feedBack.EditBox.ScrollFrame.ScrollBar)

	local browserFrame = MeetingHornMainPanel.Browser
	reskinMHDropdown(browserFrame.Activity)
	reskinMHDropdown(browserFrame.Mode)
	B.ReskinEditBox(browserFrame.Input)
	B.Reskin(browserFrame.Reset)
	B.Reskin(browserFrame.Refresh)

	local progressBar = browserFrame.ProgressBar
	...
	for i = 1, 7 do
		local header = browserFrame["Header"..i]
		if header then
			local bg = B.ReskinTab(header)
			bg:SetPoint("TOPLEFT", -5, 0)
		end
	end
	B.StripTextures(browserFrame)
	B.ReskinScroll(browserFrame.ActivityList.scrollBar)

	hooksecurefunc(browserFrame.ActivityList, "Fire", function(_, event, button)
		if event ~= "OnItemFormatting" then return end
	...
	-- activity manage
	local manageFrame = MeetingHornMainPanel.Manage
	local creatorFrame = manageFrame.Creator
	B.StripTextures(creatorFrame)
	reskinMHDropdown(creatorFrame.Activity)
	reskinMHDropdown(creatorFrame.Mode)
	B.Reskin(creatorFrame.CreateButton)
	creatorFrame.CreateButton.RightSeparator:Hide()
	B.Reskin(creatorFrame.CloseButton)
	creatorFrame.CloseButton.LeftSeparator:Hide()
	B.StripTextures(creatorFrame.Comment)
	B.CreateBDFrame(creatorFrame.Comment, .25)
	B.ReskinScroll(creatorFrame.Comment.ScrollFrame.ScrollBar)

	local chatFrame = manageFrame.Chat
	chatFrame.ChatBg:Hide()
	B.CreateBDFrame(chatFrame.ChatFrame, .25)
	B.ReskinScroll(chatFrame.ChatFrame.scrollBar)

	-- help frame
	B.StripTextures(MeetingHornMainPanel.Help)
	B.CreateBDFrame(MeetingHornMainPanel.Help, .25)

	-- options
	local optionsFrame = MeetingHornMainPanel.Options
	B.StripTextures(optionsFrame.Options)
	B.CreateBDFrame(optionsFrame.Options, .25)
	B.StripTextures(optionsFrame.Filters)
	B.CreateBDFrame(optionsFrame.Filters, .25)
	B.ReskinScroll(optionsFrame.Filters.FilterList.scrollBar)
	B.Reskin(optionsFrame.Filters.Add)
	B.Reskin(optionsFrame.Filters.Import)
	B.Reskin(optionsFrame.Filters.Export)
	...
	local dropdown = GUI and GUI:GetClass("DropMenu")
	...
	hooksecurefunc(dropdown, "Constructor", reskinMHDropdown)
	hooksecurefunc(dropdown, "Toggle", reskinMHDropdown)
end

...
L["Tutorial Page1"] = "下面將指導一些日常功能。|n|n此處將加載一些諸如|cffffcc00動作條、技能監控、團隊框架、名條|r等系統設置來改善插件的易用性。|n|n|cffff0000此頁設置無法跳過。|r"
...
L["Tutorial Page5"] = "設置完畢，點擊|cffffcc00套用|r後重載生效。|n|n|cffff0000注意:|r|n|n|cff00ccff角落的訊息條包含諸多額外的功能，請自行查看；|n|n大部分的設置在控制面板中都可以修改！|r"
L["Help Title"] = "幫助說明"
...
L["Unit Intro"] = "|n監視法術所來自的單位。|n|nplayer: 玩家單位的法術；|n|ntarget: 目標單位的法術；|n|npet: 寵物單位的法術。"
...
L["Action*"] = "行為*"
L["Action Intro"] = "|n為團隊框架設置按鍵行為。|n|n輸入target，設置目標；|n|n輸入follow，設置跟隨；|n|n輸入數字，設置施法的法術ID。此操作會固定使用最高等級的法術；|n|n你也可以直接輸入巨集。|n對於存在多行内容的巨集，用符號~表示換行。|n若要設置不同等級的法術，你只能綁定巨集（指向）。|n|n你可以使用巨集來為不同專精分別設置:|n/cast [@mouseover,spec1]XXX;[@mouseover,spec2]XXX"
L["Key*"] = "按鍵*"
L["Key Intro"] = "|n為該法術綁定一個滑鼠按鍵。|n|n為防止衝突，不建議設置單獨的左鍵或者中鍵。"
L["Modified Key"] = "功能键"
L["ModKey Intro"] = "|n為該法術設置組合鍵。"
...
L["Show ItemQuality"] = "角色面板裝備品質信息"
L["MapFader"] = "移動時淡化地圖"
L["EnhancedQuestLog"] = "增强任務列表和追蹤"
L["EnhancedQuestLogTips"] = "|n拓寬默認的任務列表，同時美化默認的任務追蹤。|n|n無需再安裝ClassicQuestLog等插件。"
L["TargetEnhancedMenu"] = "目標右鍵菜單增强"
...
L["MenuEnhancedTips"] = "|n給目標和隊友的右鍵菜單添加公會邀請和添加好友的選項。"
L["Nameplate ClassPowerOnly"] = "隱藏血條及能量條"
L["AuraWatch WatchSpellRank"] = "監控法術的不同等級"
L["ExtendedQuestLogAddons"] = "|n支援插件:|n- ClassicQuestLog|n- QuestGuru|n- QuestLogEx"
...
L["HideAllID"] = "關閉所有法術及物品信息"
L["EnergyTicker"] = "回能/回藍監控"
L["ClassColorChat"] = "强制顯示職業顔色"
L["Recount Skin"] = "啓用Recount皮膚"

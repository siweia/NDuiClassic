...
L["ActionBarTip"] = "|n關閉后，微型菜單也將一同被禁用。"
...
L["Action*"] = "行為*"
L["Action Intro"] = "|n為團隊框架設定按鍵行為。|n|n輸入target，設定目標；|n|n輸入focus，設定焦點；|n|n輸入follow，設定跟隨；|n|n輸入數字，設定施法的法術ID。此操作會固定使用最高等級的法術；|n|n你也可以直接輸入巨集。|n對於存在多行内容的巨集，用符號~表示換行。|n若要設定不同等級的法術，你只能輸入巨集（指向）。"
L["Key*"] = "按鍵*"
...
L["AutoDismountTip"] = "|n當你與飛行管理員對話後，自動解散坐騎。"

...
L["Action*"] = "行為*"
L["Action Intro"] = "|n為團隊框架設置按鍵行為。|n|n輸入target，設置目標；|n|n輸入follow，設置跟隨；|n|n輸入數字，設置施法的法術ID。此操作會固定使用最高等級的法術；|n|n你也可以直接輸入巨集。|n對於存在多行内容的巨集，用\\n來進行換行。|n若要設置不同等級的法術，你只能綁定巨集（指向）。|n|n你可以使用巨集來為不同專精分別設置:|n/cast [@mouseover,spec1]XXX;[@mouseover,spec2]XXX"
L["Key*"] = "按鍵*"

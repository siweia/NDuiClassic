...
L["Action*"] = "行为*"
L["Action Intro"] = "|n为团队框体设置按键行为。|n|n输入target，设置目标；|n|n输入follow，设置跟随；|n|n输入数字，设置施法的法术ID。此操作会固定使用最高等级的法术；|n|n你也可以直接输入宏。|n对于存在多行内容的宏命令，用符号~表示换行。|n若要设置不同等级的法术，你只能输入宏（指向）。"
L["Key*"] = "按键*"

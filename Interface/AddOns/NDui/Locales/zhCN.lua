...
L["Tutorial Page1"] = "下面将指导一些日常功能。|n|n此处将加载一些诸如|cffffcc00动作条、技能监控、团队框体、姓名板|r等系统设置以改善日常的易用性。|n|n|cffff0000此页无法跳过。|r"
L["Tutorial Page2"] = "接下来会载入|cffffcc00并锁定|r聊天窗口的大小以及位置。|n|n你之后可以在|cffffcc00控制台-聊天窗口|r里取消锁定。|n|n|cffff0000注意:|r|n|n|cff00ccff当你按住Shift键时，在聊天窗口滚动鼠标滚轮，可以快速翻至页首或页尾。|n|n底部的长条点击可以进入相应的频道。|r"
L["Tutorial Page3"] = "|cffff0000注意: 本页的设置为账号共享。|r|n|n这里会载入默认推荐的UI缩放数值，以达到最优的表现效果。|n|n你之后可以在|cffffcc00控制台-通用设置|r里选取一个适合自己的UI缩放数值。"
L["Tutorial Page4"] = "|cffff0000注意: 本页的设置为账号共享。|r|n|n最后一步，将导入Skada、DBM、BigWigs的默认位置等设定（不会锁定），以使其更符合NDui的整体风格。|n|n你也可以在|cffffcc00控制台-界面美化|r里取消它们的皮肤，一切取决于自己。"
L["Tutorial Page5"] = "设置完毕！点击|cffffcc00应用|r后重载插件生效。|n|n|cffff0000注意:|r|n|n|cff00ccff信息条的每一个模块都有额外功能，请务必查看一遍；|n|n大部分的设置在控制台中都可以修改！|r"
L["Help Title"] = "帮助说明"
...
L["ReminderTip"] = "|n根据自身职业，缺少特定Buff时进行提示。|n支持耐力/智力/战斗怒吼/野性印记/心灵之火/荆棘术。"
...
L["Unit Intro"] = "|n监控法术所来自的单位。|n|nplayer: 玩家单位的法术；|n|ntarget: 目标单位的法术；|n|npet: 宠物单位的法术。"
...
L["Action*"] = "行为*"
L["Action Intro"] = "|n为团队框体设置按键行为。|n|n输入target，设置目标；|n|n输入follow，设置跟随；|n|n输入数字，设置施法的法术ID。此操作会固定使用最高等级的法术；|n|n你也可以直接输入宏。|n对于存在多行内容的宏命令，用符号~表示换行。|n若要设置不同等级的法术，你只能绑定宏（指向）。|n|n你可以使用宏来为不同专精分别设置:|n/cast [@mouseover,spec1]XXX;[@mouseover,spec2]XXX"
L["Key*"] = "按键*"
...
L["ExtendedQuestLogAddons"] = "|n支持插件:|n- ClassicQuestLog|n- QuestGuru|n- QuestLogEx"
